...
export default new Vue({
	...
	methods: {
		register() {
			...
			if (Nick.length < 2) {
				...
				Ajaxs({ Controller: "LoginAction/Reg" }, { Phone: Phone, Pass: Pass, Nick: Nick, VCode: VCode }, {
					OkFun(response) {
		...
		getCode() {
			...
			if (!check) {
				...
				Ajaxs({ Controller: "LoginAction/GetVCode" }, { Phone: parseInt(self.telephone) }, {
					OkFun(response) {
	...
	mounted() {
		let countDown: any = localStorage.getItem("countDown");
		if (countDown) {
