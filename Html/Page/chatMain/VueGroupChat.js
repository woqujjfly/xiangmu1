define(["require", "exports", "../../Plugs/Module/Vue/Vue", "../../Plugs/Module/layer/layer", "../../Plugs/Module/WebSocketPro", "../../Plugs/Module/chatemotion", "./FileController", "../../Plugs/WinBase/global"], function (require, exports, Vue_1, layer, WebSocketPro_1, chatemotion_1, FileController_1, global_1) {
    "use strict";
    ...
    class MsgObj {
        ...
        Check() {
            ...
            if (self.Obj.Type == 0) {
                        ...
                        if (e.indexOf('data:image/png;base64') > -1) {
                            FileController_1.default.uploadBase64(e, (base64, url) => {
                                ...
                                if (++self.currentTime == self.count) {
                                    exports.VueGroupChat.send(0, self.Obj, msg);
                                }
                            });
                        }
                        else if (e.indexOf('http') != 0) {
                            FileController_1.default.upload(exports.VueGroupChat, e, (src, url) => {
                ...
                if (self.count == 0) {
                    exports.VueGroupChat.send(0, self.Obj);
                }
            }
        }
    }
    exports.VueGroupChat = new Vue_1.default({
        el: "#chatMainApp",
        data: {
        ...
        methods: {
            ...
            messageLog() {
                ...
                let historyList = DB.GetTable(self.Params.MyID, 'SELECT * FROM ChatRec WHERE (MyId=@p0 AND FriendId=@p1) OR (MyId=@p1 AND FriendId=@p0) ORDER BY MsgId desc LIMIT @p2', self.Params.MyID, self.Params.ID, '60');
                    ...
                    System.PostFile({
                        ...
                        Done(response) {
                            ...
                            if (res.ErrCode == 0) {
                                ...
                                exports.VueGroupChat.send(2, msgInfo, path);
                                self.uploadFileList = [];
                                self.isUploadFile = false;
                            }
                            else {
                                msgInfo.MsgStatus = 2;
                                msgInfo.ErrMsg = '上传失败';
                                DB.ExecuteNonQuery(exports.VueGroupChat.Params.MyID, 'UPDATE chatrec SET MsgStatus=@p0, ErrMsg=@p1 WHERE MsgId=@p2', 2, '上传失败', msgInfo.MsgId.toString());
                            }
                        },
                        ...
                        Error(err) {
                            ...
                            DB.ExecuteNonQuery(exports.VueGroupChat.Params.MyID, 'UPDATE chatrec SET MsgStatus=@p0, ErrMsg=@p1 WHERE MsgId=@p2', 2, '发送失败, 网络故障', msgInfo.MsgId.toString());
                        }
                    });
                }
                else {
                        ...
                        System.PostFile({
                            ...
                            Done(response) {
                                ...
                                if (res.ErrCode == 0) {
                                    ...
                                    exports.VueGroupChat.send(2, msgInfo, path[i]);
                                }
                                else {
                                    msgInfo.MsgStatus = 2;
                                    msgInfo.ErrMsg = '上传失败';
            ...
            msg_contextmenu(item, index) {
                ...
                Menu.Popup({
                    ...
                    Click: (Key, Checked) => {
                        ...
                        switch (Key) {
                            case '复制':
                                document.execCommand("copy", false, item.Msg);
                                break;
                            case '全选':
                                document.execCommand("selectAll", false, item.Msg);
                                break;
                            case '撤回':
                                ;
                                break;
            ...
            send(type, msgInfo, msg) {
                ...
                if (msgInfo) {
                    ...
                    DB.ExecuteNonQuery(self.Params.MyID, 'INSERT INTO GroupMsg VALUES(@p0, @p1, @p2, @p3, @p4, @p5, @p6)', msgInfo.MsgId.toString(), self.Params.MyID, self.Params.ID, type, msgInfo.Msg, 0, '');
                }
                let send = () => {
                    WebSockets.Send("SendQMsg", { Type: type, Msg: msg, To: Number(self.Params.ID), MsgId: new Date().getTime() }, (msg) => {
                        if (msg.ErrCode == 0) {
                            if (msgInfo) {
                                msgInfo.MsgStatus = 1;
    ...
    let chatList;
    if (exports.VueGroupChat.Params.UnRead) {
        if (exports.VueGroupChat.Params.UnRead > 25) {
            exports.VueGroupChat.Params.UnRead = 25;
        }
        chatList = DB.GetTable(exports.VueGroupChat.Params.MyID, 'SELECT * FROM GroupMsg WHERE (MyId=@p0 AND GroupId=@p1) OR (MyId=@p1 AND GroupId=@p0) ORDER BY MsgId desc LIMIT @p2', exports.VueGroupChat.Params.MyID, exports.VueGroupChat.Params.ID, exports.VueGroupChat.Params.UnRead);
    }
    else {
        chatList = DB.GetTable(exports.VueGroupChat.Params.MyID, 'SELECT * FROM GroupMsg WHERE (MyId=@p0 AND GroupId=@p1) OR (MyId=@p1 AND GroupId=@p0) ORDER BY MsgId desc LIMIT @p2', exports.VueGroupChat.Params.MyID, exports.VueGroupChat.Params.ID, 3);
    }
    ...
    function getRecord(arr, type) {
        if (arr.length > 0) {
            ...
            for (let i = 0, l = arr.length; i < l; i++) {
                ...
                PushResult(exports.VueGroupChat.Params.ID, {
                    Msg: msg,
                    Type: arr[i].Type,
                    Dir: arr[i].MyId == exports.VueGroupChat.Params.MyID ? 0 : 1,
            ...
            if (exports.VueGroupChat.moreTime > 0 && arr.length < 25) {
                exports.VueGroupChat.lookMore = false;
            }
            else {
                exports.VueGroupChat.lookMore = true;
            }
        }
    }
    function PushResult(userId, info, type, insertType) {
        if (info.Type == 3) {
            ...
            let list = type ? exports.VueGroupChat.historyList : exports.VueGroupChat.chatList;
            if (!list[userId]) {
                list[userId] = [];
            ...
            type == 'history' ? exports.VueGroupChat.historyList = Object.assign({}, list) : exports.VueGroupChat.chatList = Object.assign({}, list);
        }
